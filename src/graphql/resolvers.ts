  ...
  User: {
    orders: async (parent, args, { currentUser }) => {
      ...
      try {
        ...
        const orders = await OrderModel.find(isOwer ? { id: currentUser.id } : {})
        return orders;
      } catch (error) {
        throw new GraphQLError(`Ha ocurrido algo al intentar recuperar los pedidos: ${error}`);
      }
    },
  },
  CartItem: {
  ...
  Query: {
    ...
    getOrderById: async (_, { id }) => {
      try {
        ...
        return new GraphQLError(
          `Error al encontrar la orden: ${error.message}`
        );
      }
    },
  },
